...
### Regions of DNA on chromosomes that guard against cell death {.fragment}
...
### Shortening associated with cardiometabolic outcomes {.fragment}
...
[**Note:** Race is not 'manipulable' like a traditional treatment/exposure]{.fragment}
...
- $\mathcal{P}$: Population distribution of $(\class{highlight-prop}{A},\ \class{highlight-gray}{\boldsymbol{X}},\ \class{highlight-green}{Y})$
- $\mathcal{Q}$: Sample distribution given $\class{highlight-selection}{S = 1}$
...
$$= \class{highlight-prop}{e_a^{\mathcal{P}}(\boldsymbol{X})}  \times \class{highlight-selection}{\pi(A, \boldsymbol{X})}$$
...
$$= \class{highlight-prop}{e_a^{\mathcal{Q}}(\boldsymbol{X})}  \times \class{highlight-selection}{\pi(\boldsymbol{X})}$$
...
$$\mu(a) = \mathbb{E}_{\mathcal{P}}\left[\mathbb{E}(\class{highlight-green}{Y} \mid\ \class{highlight-prop}{A = a},\ \class{highlight-gray}{\boldsymbol{X}})\right] = \mathbb{E}_{\mathcal{Q}}\left[\class{highlight-green}{g_a(\boldsymbol{X})}\ \class{highlight-selection}{w(\boldsymbol{X})}\right],$$

where $\class{highlight-green}{g_a(\boldsymbol{X})} = \mathbb{E}_{\mathcal{Q}}(\class{highlight-green}{Y} \mid\ \class{highlight-prop}{A = a},\ \class{highlight-gray}{\boldsymbol{X}})$ and $\class{highlight-selection}{w(\boldsymbol{X})}$ is a known selection factor, we have
$$\text{ACD} = \mu(1) - \mu(0)$$
:::
...
::: {.fragment}
**Interpretation:** The population-standardized, covariate-balanced difference in mean telomere length between Black and White individuals
:::
...
$$\hat\mu_{IPW1}(a) = \frac{1}{n}\sum_{i=1}^n \frac{\class{highlight-prop}{I(A_i=a)}}{\class{highlight-prop}{\hat e_a^\mathcal{P}(\boldsymbol{X}_i)}} \times \class{highlight-green}{Y_i} \times \frac{\class{highlight-selection}{\Pr(S = 1)}}{\class{highlight-selection}{\pi(A_i, \boldsymbol{X}_i)}}$$
:::
...
$$\hat\mu_{IPW2}(a) = \frac{1}{n}\sum_{i=1}^n \frac{\class{highlight-prop}{I(A_i=a)}}{\class{highlight-prop}{\hat e_a^\mathcal{Q}(\boldsymbol{X}_i)}} \times \class{highlight-green}{Y_i} \times \frac{\class{highlight-selection}{\Pr(S = 1)}}{\class{highlight-selection}{\pi(\boldsymbol{X}_i)}}$$
:::
...
$$\hat\mu_{AIPW}(a) = \frac{1}{n}\sum_i \frac{\class{highlight-selection}{\Pr(S = 1)}}{\class{highlight-selection}{\pi(\boldsymbol{X}_i)}}\times \left[\frac{\class{highlight-prop}{I(A_i=a)}}{\class{highlight-prop}{\hat e_a^\mathcal{Q}(\boldsymbol{X}_i)}} \class{highlight-green}{\{Y_i - \hat g_a(X_i)\}+\hat g_a(X_i)}\right]$$
:::

## How Existing Methods Compare

<br>

| Method | Addresses Confounding? | Addresses Selection? |
|------------------|:-----------------:|:-----------------:|
Existing Approaches                                 |          |     |
...
All methods perform comparably for bias, MSE, and coverage
:::
...
Simple regression performs poorly; methods adjusting for $A$ and $\boldsymbol{X}$ perform well
:::
...
Approaches that do not survey weight have bias and poor coverage
:::
...
Matches our data setting; proposed estimators outperform current approaches
:::
...
- ACD estimates attenuate as we make appropriate adjustments for confounding and selection bias
  - Linear regression: ACD estimate of 0.0265, 95% CI: 0.0106–0.0423
  - Proposed approaches: 0.0132 to 0.0176, failing to detect a statistically significant difference
- Results suggest a confounding relationship between SES and race
- Methods incorporating the NHANES stratified, clustered design tended to have more conservative standard error estimates
...
flowchart LR
  Q["Define<br/>target estimand"] --> R{"Sample or<br/>population?"}

  R -->|Sample| SB{"Are covariates<br/>balanced?"}
  SB -->|Yes| SN["Estimate sample<br/>contrast"]
  SB -->|No| SP{"Use propensity<br/>scores?"}
  SP -->|Yes| S["Balance groups<br/>within sample"]
  SP -->|No| SA["Use outcome model<br/>or report caveat"]

  R -->|Population| T{"Does selection<br/>depend on A or X?"}
  T -->|No / ignorable| U["Survey<br/>standardization"]
  T -->|Yes| V["Choose OM, IPW1,<br/>IPW2, or AIPW"]

  SN --> X["Variance estimation<br/>for chosen estimand"]
  S --> X
  SA --> X
  U --> X
  V --> X
  ...
  classDef caution fill:#FAECEB,stroke:#A7433B,color:#18212B,font-size:10px;
  classDef green fill:#E8F4EF,stroke:#5D9684,color:#18212B,font-size:10px;

  class Q,R,SB,SP,T,V focus;
  class SN,S,U,X neutral;
  class SA caution;
```

## Planning Similar Analyses

1. Start by naming the estiman (e.g., ACD)
...
- NHANES contains a wealth of data to inform policy-relevant research
- Only valid when the estimand, adjustment strategy, and survey design are aligned
- We propose an approach for estimating controlled outcome differences in a particular setting
- The proposal minimizes bias and achieves correct inference compared to standard analytic methods
- The context of racial disparities presents these challenges in a way that should be rigorously studied for best practice recommendations
